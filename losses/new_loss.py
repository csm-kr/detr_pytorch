...
class HungarianLoss(nn.Module):
    ...
    def make_target(self, outputs, boxes, labels):
        ...
        # self.num_classes : 91
        target_classes = torch.full([batch_size, 100], self.num_classes, dtype=torch.int64, device=device)
        target_boxes = torch.full([batch_size, 100, 4], -1, dtype=torch.float32, device=device)
